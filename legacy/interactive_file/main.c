        ...
        if (mode == 1) {
            ...
            if (argc == 4) {
                ...
                if (check_if_current_players_penguins_can_move(players[current_player], board)){
                    display_board_file_format(board, rows, columns);
                    ask_for_penguin_coordinates(players[current_player], board);
                    ask_for_coordinates_to_move_penguin(players[current_player], board);
                    move_penguin(board, players[current_player], x_penguin, y_penguin, x, y);
                } else {
                ...
                printf("Wrong amount of commandline parameters "
                "they should be of this format:\n'penguins.exe "
                "phase=movement inputboard.txt outpuboard.txt'");
                return 2;
            }
        } else if (mode == 3) {
            printf("%s\n", player_name);
